...
### 4.3 从 NextCloud 下载流程

1. 在资源管理器中右键目标文件或目录，选择 **“Download From NextCloud To Local”**（或通过命令面板执行）。
2. 插件会根据本地路径自动推荐远程目录（已去除工作区根目录），如有需要可手动调整。
3. 连接至 NextCloud，检查远程目录并显示下载进度。
4. 如本地已存在同名文件/目录，弹出覆盖确认提示，可选择继续或取消。
5. 下载结束后在通知与日志面板查看结果。
...
💡 **提示**：默认会预填当前文档的相对路径（已去除工作区根目录），可直接调整为对应的 NextCloud 路径。
...
1. 在资源管理器中右键文件或目录 → 选择 **Download From NextCloud To Local**。
2. 目录选择框会自动填入对应的远程路径建议（基于本地相对路径），可按需修改。
3. 若本地已存在同名文件/目录，系统会弹出确认提示：
   - **目录下载**：显示 "Overwrite"（覆盖所有文件）、"Skip"（跳过已有文件，仅下载缺失的）、"Cancel"（取消）
   - **文件下载**：显示 "Overwrite"（覆盖）、"Cancel"（取消）
4. 进度与结果会显示在通知栏和日志面板中。

> 支持整目录递归下载，便于从 NextCloud 回同步最新资料。选择 "Skip" 选项时，会自动跳过已存在的文件，仅下载缺失的文件。
...
**最后更新**: 2025-11-10  
**主要更新**:
- 🐛 修复：识别 `./assets/` 相对路径的 Markdown 资源并正确上传
- 🧭 上传目录默认建议本地相对路径，避免手动输入
- 📁 发布完成提示新增"打开目录"，并复用已有分享链接
- ⬇️ 新增"Download From NextCloud To Local"，支持整目录递归下载
- 🔧 优化下载确认提示：目录下载支持 "Skip" 选项，自动跳过已有文件
